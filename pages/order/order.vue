			...
			<swiper-item class="tab-content" v-for="(tabItem,tabIndex) in navList" :key="tabIndex">
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					...
					<view v-for="(item,index) in tabItem.orderList" :key="index" class="order-item">
						...
						<view>
							<image class="goods-img" :src='item.productDetail.image' mode="widthFix" style="width: 50px;"></image>
							<view class="right">
								<text class="title clamp">{{item.productDetail.productName}}</text>
								<text class="attr-box">{{item.styles[0].styleName}} x {{item.styles[1].styleName}}</text>
						...
						<view class="price-box">
							...
							件商品 实付款
							<text class="price">{{item.productDetail.productPrice}}</text>
						</view>
		...
		methods: {
			...
			loadData(source) {
				...
				setTimeout(() => {
					...
					uni.request({
						url: this.myroot + "getOrderListBycid",
						data: map,
						dataType: "json",
						method: "POST",
						success: (res) => {
							...
							for (var i = 0; i < orderList.length; i++)
								orderList[i].productDetail.image = this.myroot + "upload/" + orderList[i].productDetail.image
